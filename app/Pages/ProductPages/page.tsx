  ...
  return (
    <Box className=" flex items-center gap-x-[5px]">
      <IconButton
        aria-label=""
        onClick={() =>
          router.push(`/../../main_pages/Dashboard/AddProduct?ProductId=${id}`)
        }
        icon={
          <svg
            width="16"
...
function Page() {
  ...
  const column = [
    {
      ...
      cell: (row: productType) => (
        <div className=" flex items-center gap-x-[5px] gap-y-[5px] pt-[5px] pb-[5px]">
          ...
          <Box>
            <Text className=" text-[#007460]">{row?.name || ""}</Text>
          </Box>
        </div>
      ),
    },
    {
      name: "Price",
      cell: (row: productType) => (
        <Text className="text-[12px]">{row?.price || ""}</Text>
      ),
    },
    {
      name: "Status",
  ...
  return (
    <Box>
      <Navbar />
      <Box className=" w-10/12 m-auto mt-[40px]">
        <Box
          border="1px"
